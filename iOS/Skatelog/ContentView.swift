    ...
    private func row(_ entry: SessionEntry) -> some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Attempts: \(entry.value1)  \u{00B7}  Landed: \(entry.value2)")
                    .font(.caption)
...
struct EntrySheet: View {
    ...
    @State private var option: String = SkatelogOptions.all.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Trick") {
                    Picker("Trick", selection: $option) {
                        ForEach(SkatelogOptions.all, id: \.self) { t in
                            Text(t).tag(t)
                        }
                    }
                    .pickerStyle(.menu)
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        if var existing = entry {
                            existing.trick = option
                            existing.value1 = Int(value1)
                            existing.value2 = Int(value2)
                            existing.note = note
                            store.update(existing)
                        } else {
                            store.add(trick: option, value1: Int(value1), value2: Int(value2), note: note)
                        }
                        dismiss()
                    }
